...
func TestDecodeResponseUsageNormalization(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "null optional usage details are ignored",
		...
		{name: "null completion", usageField: `,"usage":{"completion_tokens":null}`, wantField: usage.UsageNormalizationFieldOutputTokens, wantReason: usage.UsageNormalizationReasonNull},
